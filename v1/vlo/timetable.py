...
def GetNextLesson(klass:str, groups:List[str], style:NextStyle, notext:str=""):
	now = datetime.datetime.now()

	if now.weekday() < 5:
		resp = GetTimetableData(klass.value)[now.weekday()]
		...
		if not lowest:
			return notext
		if style.value == NextStyle.Default.value:
			return f"{lowest[0]['subject_short']}"
		if style.value == NextStyle.Detailed.value:
			return f"{lowest[0]['subject']}"
		if style.value == NextStyle.Timed.value:
...
print(json.dumps(GetTimetableData("2e", 0, True), ensure_ascii=False, indent=1), file = open("a.txt", "w", encoding="utf8"))